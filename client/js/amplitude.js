...
var Amplitude = (function () {
	...
	*/
	...
	function publicReturnActiveIndex () {
		if (config.shuffle_on) {
			var trackName = config.shuffle_list[config.shuffle_active_index].name
			var matchingId
			for (i = 0; i < config.songs.length; i++) {
				if (config.songs[i].name === trackName) {
					matchingId = i
				}
			}
			return matchingId
	...
	*/
	return {
		...
		getIndex: publicReturnActiveIndex
	};
